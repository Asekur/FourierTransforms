...
class SignalPolyharmonic {
    ...
    func restore(amplitudes: [Double], phases: [Double], excludingPhases: Bool) -> [Double] {
        ...
        for i in 0..<Constants.frameCount {
            var tempRestore = 0.0
            for j in 0..<length {
                var angle = (2.0 * Double.pi * Double(j) * Double(i)) / Double(Constants.frameCount)
                if !excludingPhases {
                    angle -= phases[j]
                }
                tempRestore += amplitudes[j] * cos(angle)
            }
            restore.append(amplitudes[0] / 2.0 + tempRestore)
        }
        return restore
    }
